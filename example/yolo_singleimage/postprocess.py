...
def scale_coords(boxes, input_shape, image_shape):
    """ 将检测框坐标从输入尺寸缩放到原始图像尺寸 """
    # 计算缩放比例 (高度, 宽度)
    gain = min(input_shape[0] / image_shape[0], input_shape[1] / image_shape[1])
    pad = ((input_shape[1] - image_shape[1] * gain) / 2,  # x padding
           (input_shape[0] - image_shape[0] * gain) / 2)  # y padding
    ...
    # 裁剪坐标到图像边界
    boxes[:, [0, 2]] = boxes[:, [0, 2]].clip(0, image_shape[1])  # x
    boxes[:, [1, 3]] = boxes[:, [1, 3]].clip(0, image_shape[0])  # y
    return boxes
...
# ================= 主处理流程 =================
def process_outputs(outputs, image_shape):
    """ 处理所有三个输出层 """
    all_boxes = []
    all_scores = []
    all_class_ids = []
    ...
    for i, (output, anchors) in enumerate(zip(outputs, ANCHORS)):
        ...
        # 打印解码后的目标数量
        print(f"Layer {i}: {len(boxes)} boxes before filtering")
        ...
        # 打印过滤后的目标数量
        print(f"Layer {i}: {np.sum(mask)} boxes after filtering")
    ...
    # 打印最终目标数量
    print(f"Total boxes after NMS: {len(final_boxes)}")
    
    # 坐标缩放到原始图像尺寸
    final_boxes = scale_coords(final_boxes, IMG_SIZE, image_shape)
